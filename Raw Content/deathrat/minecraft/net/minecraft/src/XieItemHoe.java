    	...
    	for (int x=-d; x<=d; x++)
    		for (int y=-d; y<=d; y++)
    			for (int z=-d; z<=d; z++) {
    				...
    				if (XieMod.Farming.Enable.pumpkins && blockID==Block.pumpkin.blockID && world.rand.nextInt(n)==0) return XieMod.pumpkinSeeds;
    				if (XieMod.Farming.Enable.melons && blockID==XieMod.watermelon.blockID && world.rand.nextInt(n)==0) return XieMod.melonSeeds;
